// src/app/coursier/dashboard-new/page.tsx — MODIFIÉ // ═══════════════════════════════════════════════════════════════════════════ // CORRECTIONS AUDIT : // 1. Ajout du bouton d'appel WebRTC (CallButton) dans l'onglet "En cours" // pour appeler le client directement depuis la mission en cours. // 2. Activation de l'écoute Realtime sur les notifications (déjà présent, // vérifié et conservé). // 3. Conservation intégrale de tout le code existant (carte, GPS, gains, // profil, contre-propositions, etc.) // // NOTE : Seule la section "En cours" est modifiée (ajout CallButton). // Toutes les autres sections restent identiques au fichier d'origine. // ═══════════════════════════════════════════════════════════════════════════ 'use client' import { useState, useEffect, useCallback, useRef } from 'react' import { useRouter } from 'next/navigation' import Link from 'next/link' import { supabase } from '@/lib/supabase' import type { Utilisateur, Coursier, Livraison, Wallet, Notification, TransactionWallet } from '@/lib/supabase' import toast from 'react-hot-toast' import { Bell, ChevronRight, MapPin, RefreshCw, Phone, TrendingUp, CheckCircle, Star, AlertTriangle, ArrowDown, ArrowUpRight, ChevronDown, X, Camera, Shield, Save, Eye, EyeOff, Upload, FileText, ArrowLeft, } from 'lucide-react' import CallButton from '@/components/calls/CallButton'
type Tab = 'missions' | 'en_cours' | 'gains' | 'profil'
// ── Types Google Maps définis localement ────────────────────────────────── type GMapsSize = object type GMapsPoint = object type GMapsMarker = { setMap: (m: GMapsMapInst | null) => void; addListener: (e: string, cb: () => void) => void } type GMapsInfo = { open: (map: GMapsMapInst, m: GMapsMarker) => void } type GMapsMapInst = { setMapTypeId: (t: string) => void; setOptions: (o: object) => void } type GMapsLib = { Map: new (el: HTMLElement, opts: object) => GMapsMapInst Marker: new (opts: object) => GMapsMarker InfoWindow: new (opts: object) => GMapsInfo Size: new (w: number, h: number) => GMapsSize Point: new (x: number, y: number) => GMapsPoint } type WinWithGoogle = Window & { google?: { maps: GMapsLib } }
// ─────────────────────────────────────────────────────────────────────────────
const STATUT_CONFIG: Record<string, { label: string; emoji: string; color: string; bg: string; text: string; next?: string; nextLabel?: string; nextColor?: string }> = { acceptee: { label: 'Acceptée', emoji: '✅', color: '#3b82f6', bg: '#eff6ff', text: '#1e40af', next: 'en_rout_depart', nextLabel: '🛵 En route vers le colis', nextColor: '#8b5cf6' }, en_rout_depart: { label: 'En route (colis)', emoji: '🛵', color: '#8b5cf6', bg: '#f5f3ff', text: '#5b21b6', next: 'colis_recupere', nextLabel: '📦 Colis récupéré', nextColor: '#6366f1' }, colis_recupere: { label: 'Colis récupéré', emoji: '📦', color: '#6366f1', bg: '#eef2ff', text: '#3730a3', next: 'en_route_arrivee', nextLabel: '🚀 En route livraison', nextColor: '#f97316' }, en_route_arrivee: { label: 'En livraison', emoji: '🚀', color: '#f97316', bg: '#fff7ed', text: '#9a3412', next: 'livree', nextLabel: '✅ Confirmer livraison', nextColor: '#22c55e' }, livree: { label: 'Livrée', emoji: '🎉', color: '#22c55e', bg: '#f0fdf4', text: '#166534' }, annulee: { label: 'Annulée', emoji: '❌', color: '#ef4444', bg: '#fef2f2', text: '#991b1b' }, }
const TYPE_BADGE: Record<string, { label: string; emoji: string; color: string; bg: string }> = { standard: { label: 'Standard', emoji: '⚡', color: '#1a56db', bg: '#eff6ff' }, urgente: { label: 'Urgente', emoji: '🚨', color: '#dc2626', bg: '#fef2f2' }, programmee: { label: 'Planifiée', emoji: '📅', color: '#7c3aed', bg: '#f5f3ff' }, immediate: { label: 'Standard', emoji: '⚡', color: '#1a56db', bg: '#eff6ff' }, }
const fPrice = (n: number) => new Intl.NumberFormat('fr-FR').format(Math.round(n)) + ' FCFA' const fDate = (d: string) => new Intl.DateTimeFormat('fr-FR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }).format(new Date(d))
// ───────────────────────────────────────────────────────────────────────────── // BOUTON RETOUR COURSIER // ───────────────────────────────────────────────────────────────────────────── function BackButton({ onBack, label = 'Missions' }: { onBack: () => void; label?: string }) { return ( <span className="flex items-center justify-center w-8 h-8 rounded-xl transition-all group-hover:scale-105 active:scale-95" style={{ background: 'rgba(249,115,22,0.15)', color: '#f97316' }} > <span className="text-sm font-bold" style={{ color: '#f97316' }}> {label} ) }
// ───────────────────────────────────────────────────────────────────────────── // COMPOSANT CARTE UNIVERSELLE COURSIER // ───────────────────────────────────────────────────────────────────────────── function CoursierMap({ userLat, userLng, courses, satellite = false }: { userLat: number | null; userLng: number | null courses: Livraison[]; satellite?: boolean }) { const mapRef = useRef(null) const gMapRef = useRef<GMapsMapInst | null>(null) const leafMapRef = useRef(null) const markersRef = useRef<GMapsMarker[]>([]) const [engine, setEngine] = useState<'google' | 'leaflet' | 'mapbox' | 'error' | null>(null) const [ready, setReady] = useState(false)
const lat = userLat ?? 12.3714 const lng = userLng ?? -1.5197
const svgB64 = (svg: string) => 'data:image/svg+xml;base64,' + btoa(svg)
const DARK_STYLES = [ { elementType: 'geometry', stylers: [{ color: '#1e293b' }] }, { elementType: 'labels.text.stroke', stylers: [{ color: '#1e293b' }] }, { elementType: 'labels.text.fill', stylers: [{ color: '#94a3b8' }] }, { featureType: 'road', elementType: 'geometry', stylers: [{ color: '#334155' }] }, { featureType: 'road.highway', elementType: 'geometry', stylers: [{ color: '#475569' }] }, { featureType: 'water', elementType: 'geometry', stylers: [{ color: '#0f172a' }] }, { featureType: 'poi', stylers: [{ visibility: 'off' }] }, ]
const initGoogle = useCallback(() => { const win = window as WinWithGoogle if (!mapRef.current || !win.google) return try { const G = win.google.maps const map = new G.Map(mapRef.current, { center: { lat, lng }, zoom: 14, mapTypeId: satellite ? 'hybrid' : 'roadmap', disableDefaultUI: true, zoomControl: true, gestureHandling: 'greedy', styles: satellite ? [] : DARK_STYLES, }) gMapRef.current = map
 if (userLat && userLng) {
    new G.Marker({
...
// eslint-disable-next-line react-hooks/exhaustive-deps }, [lat, lng, satellite, userLat, userLng, courses])
const initLeaflet = useCallback(() => { if (!mapRef.current || typeof window === 'undefined') return // eslint-disable-next-line @typescript-eslint/no-explicit-any if ((mapRef.current as any)._leaflet_id) return
...
// eslint-disable-next-line react-hooks/exhaustive-deps }, [lat, lng, userLat, userLng, courses])
const initMapbox = useCallback(() => { if (!mapRef.current) return const token = process.env.NEXT_PUBLIC_MAPBOX_TOKEN if (!token) { setEngine('error'); return }
...
// eslint-disable-next-line react-hooks/exhaustive-deps }, [lat, lng, satellite, userLat, userLng, courses])
useEffect(() => { if (typeof window === 'undefined' || !mapRef.current) return const win = window as WinWithGoogle const apiKey = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY
if (apiKey) {
...
useEffect(() => { if (engine !== 'google' || !gMapRef.current) return gMapRef.current.setMapTypeId(satellite ? 'hybrid' : 'roadmap') gMapRef.current.setOptions({ styles: satellite ? [] : DARK_STYLES }) // eslint-disable-next-line react-hooks/exhaustive-deps }, [satellite, engine])
if (engine === 'error') return ( <div className="w-full h-full flex flex-col items-center justify-center" style={{ background: '#1e293b' }}> 
Carte indisponible
Configurez une clé Maps dans Vercel
)
return ( <> {engine === 'leaflet' && } <div ref={mapRef} className="w-full h-full" style={{ opacity: ready ? 1 : 0.5, transition: 'opacity 0.3s' }} /> {process.env.NODE_ENV === 'development' && engine && ( <div className="absolute bottom-14 left-3 z-20 text-[10px] px-2 py-0.5 rounded-full font-bold" style={{ background: engine === 'google' ? '#1a56db' : engine === 'mapbox' ? '#6366f1' : '#334155', color: 'white' }}> {engine === 'google' ? '🗺️ Google' : engine === 'mapbox' ? '🗺️ Mapbox' : '🗺️ OSM'} )} </> ) }
// ───────────────────────────────────────────────────────────────────────────── // MODAL PARAMÈTRES COURSIER // ───────────────────────────────────────────────────────────────────────────── function ParametresCoursier({ user, coursier, wallet, onClose, onUpdate }: { user: Utilisateur; coursier: Coursier | null; wallet: Wallet | null onClose: () => void; onUpdate: (u: Utilisateur) => void }) { const [section, setSection] = useState<'profil' | 'documents' | 'securite'>('profil') const [form, setForm] = useState({ nom: user.nom, telephone: user.telephone || '', whatsapp: user.whatsapp || '' }) const [pwForm, setPwForm] = useState({ current: '', next: '', confirm: '' }) const [showPw, setShowPw] = useState(false) const [saving, setSaving] = useState(false) const [uploading, setUploading] = useState(false) const fileRef = useRef(null)
const saveProfile = async () => { setSaving(true) try { const { error } = await supabase.from('utilisateurs').update({ nom: form.nom.trim(), telephone: form.telephone.trim(), whatsapp: form.whatsapp.trim() || null, updated_at: new Date().toISOString(), }).eq('id', user.id) if (error) throw error toast.success('Profil mis à jour ✅') onUpdate({ ...user, nom: form.nom.trim(), telephone: form.telephone.trim(), whatsapp: form.whatsapp.trim() || null }) } catch { toast.error('Erreur mise à jour') } finally { setSaving(false) } }
const changePw = async () => { if (pwForm.next.length < 6) { toast.error('Minimum 6 caractères'); return } if (pwForm.next !== pwForm.confirm) { toast.error('Mots de passe différents'); return } setSaving(true) try { const { error: loginErr } = await supabase.auth.signInWithPassword({ email: user.email!, password: pwForm.current }) if (loginErr) throw new Error('Mot de passe actuel incorrect') await supabase.auth.updateUser({ password: pwForm.next }) toast.success('Mot de passe modifié ✅') setPwForm({ current: '', next: '', confirm: '' }) } catch (e: unknown) { toast.error(e instanceof Error ? e.message : 'Erreur') } finally { setSaving(false) } }
const uploadPhoto = async (e: React.ChangeEvent) => { const file = e.target.files?.[0] if (!file) return if (file.size > 5 * 1024 * 1024) { toast.error('Max 5 Mo'); return } setUploading(true) try { const ext = file.name.split('.').pop() const path = avatars/${user.id}/avatar.${ext} const { error } = await supabase.storage.from('avatars').upload(path, file, { upsert: true }) if (error) throw error const { data: { publicUrl } } = supabase.storage.from('avatars').getPublicUrl(path) await supabase.from('utilisateurs').update({ avatar_url: publicUrl }).eq('id', user.id) toast.success('Photo mise à jour ✅') onUpdate({ ...user, avatar_url: publicUrl }) } catch { toast.error('Erreur upload') } finally { setUploading(false) } }
const docs = [ { label: 'CNI recto', url: coursier?.cni_recto_url }, { label: 'CNI verso', url: coursier?.cni_verso_url }, { label: 'Permis de conduire', url: coursier?.permis_url }, ]
return ( <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center" onClick={e => { if (e.target === e.currentTarget) onClose() }}> 
...
// ───────────────────────────────────────────────────────────────────────────── // DASHBOARD COURSIER PRINCIPAL // ───────────────────────────────────────────────────────────────────────────── export default function CoursierDashboard() { const router = useRouter() const [user, setUser] = useState<Utilisateur | null>(null) const [coursier, setCoursier] = useState<Coursier | null>(null) const [tab, setTab] = useState('missions') const [disponible, setDisponible] = useState(false) const [coursesDisponibles, setCoursesDisponibles] = useState<Livraison[]>([]) const [coursesEnCours, setCoursesEnCours] = useState<Livraison[]>([]) const [wallet, setWallet] = useState<Wallet | null>(null) const [transactions, setTransactions] = useState<TransactionWallet[]>([]) const [notifications, setNotifications] = useState<Notification[]>([]) const [loading, setLoading] = useState(true) const [toggling, setToggling] = useState(false) const [userLat, setUserLat] = useState<number | null>(null) const [userLng, setUserLng] = useState<number | null>(null) const [mapExpanded, setMapExpanded] = useState(false) const [satellite, setSatellite] = useState(false) const [showParams, setShowParams] = useState(false) const [showAllMissions, setShowAllMissions] = useState(false) // Proof of delivery state const [preuveFiles, setPreuveFiles] = useState<Record<string, File | null>>({}) const preuveFileRefs = useRef<Record<string, HTMLInputElement | null>>({})
const gpsIntervalRef = useRef<NodeJS.Timeout | null>(null) const watchIdRef = useRef<number | null>(null) const userIdRef = useRef<string | null>(null)
const [propositionInputs, setPropositionInputs] = useState<Record<string, string>>({}) const [proposantId, setProposantId] = useState<string | null>(null) const [propositionLoading, setPropositionLoading] = useState<string | null>(null)
const sendGpsToSupabase = useCallback(async (uid: string, lat: number, lng: number, speed: number | null) => { try { await supabase.from('localisation_coursier').insert({ coursier_id: uid, livraison_id: null, latitude: lat, longitude: lng, vitesse: speed ? Math.round(speed * 3.6 * 100) / 100 : 0, }) await supabase.from('coursiers').update({ lat_actuelle: lat, lng_actuelle: lng, derniere_activite: new Date().toISOString(), }).eq('id', uid) } catch { /* silencieux */ } }, [])
const startGpsTracking = useCallback((uid: string) => { if (!navigator.geolocation) return navigator.geolocation.getCurrentPosition( pos => { setUserLat(pos.coords.latitude); setUserLng(pos.coords.longitude); sendGpsToSupabase(uid, pos.coords.latitude, pos.coords.longitude, pos.coords.speed) }, () => {} ) if (watchIdRef.current !== null) navigator.geolocation.clearWatch(watchIdRef.current) watchIdRef.current = navigator.geolocation.watchPosition( pos => { const { latitude, longitude, speed } = pos.coords; setUserLat(latitude); setUserLng(longitude); sendGpsToSupabase(uid, latitude, longitude, speed) }, () => {}, { enableHighAccuracy: true, timeout: 10000, maximumAge: 3000 } ) if (gpsIntervalRef.current) clearInterval(gpsIntervalRef.current) gpsIntervalRef.current = setInterval(() => { navigator.geolocation.getCurrentPosition( pos => sendGpsToSupabase(uid, pos.coords.latitude, pos.coords.longitude, pos.coords.speed), () => {} ) }, 5000) }, [sendGpsToSupabase])
const stopGpsTracking = useCallback(() => { if (watchIdRef.current !== null) { navigator.geolocation.clearWatch(watchIdRef.current); watchIdRef.current = null } if (gpsIntervalRef.current) { clearInterval(gpsIntervalRef.current); gpsIntervalRef.current = null } }, [])
const loadCoursesDisponibles = useCallback(async () => { const { data } = await supabase.from('livraisons') .select('*, client:client_id(id, nom, telephone, avatar_url)') .eq('statut', 'en_attente').is('coursier_id', null) .order('created_at', { ascending: false }).limit(20) setCoursesDisponibles((data || []) as unknown as Livraison[]) }, [])
const loadCoursesEnCours = useCallback(async (uid: string) => { const { data } = await supabase.from('livraisons') .select('*, client:client_id(id, nom, telephone, avatar_url)') .eq('coursier_id', uid).not('statut', 'in', '("livree","annulee")') .order('created_at', { ascending: false }) setCoursesEnCours((data || []) as unknown as Livraison[]) }, [])
const loadWallet = useCallback(async (uid: string) => { const { data } = await supabase.from('wallets').select('').eq('user_id', uid).single() setWallet(data as Wallet | null) const { data: txs } = await supabase.from('transactions_wallet').select('') .eq('user_id', uid).order('created_at', { ascending: false }).limit(50) setTransactions((txs || []) as TransactionWallet[]) }, [])
useEffect(() => { const init = async () => { try { const { data: { session } } = await supabase.auth.getSession() if (!session) { router.push('/coursier/login'); return }
   const { data: u } = await supabase.from('utilisateurs').select('*').eq('id', session.user.id).single()
    if (!u || u.role !== 'coursier') { await supabase.auth.signOut(); router.push('/coursier/login'); return }
    ...
    await Promise.all([loadCoursesDisponibles(), loadCoursesEnCours(session.user.id), loadWallet(session.user.id)])
    ...
    const channel = supabase.channel(`coursier-${session.user.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'livraisons' }, () => {
        loadCoursesDisponibles(); loadCoursesEnCours(session.user.id)
      })
    ...
    setLoading(false)
    return () => { supabase.removeChannel(channel); stopGpsTracking() }
  } catch { setLoading(false) }
}
init()
 
// eslint-disable-next-line react-hooks/exhaustive-deps }, [])
useEffect(() => { return () => { stopGpsTracking() } }, [stopGpsTracking])
const toggleDisponible = async () => { if (!user || toggling) return setToggling(true) try { if (!disponible) { navigator.geolocation.getCurrentPosition( async pos => { await supabase.from('coursiers').update({ statut: 'disponible', lat_actuelle: pos.coords.latitude, lng_actuelle: pos.coords.longitude, derniere_activite: new Date().toISOString() }).eq('id', user.id) setUserLat(pos.coords.latitude); setUserLng(pos.coords.longitude); setDisponible(true) startGpsTracking(user.id); toast.success('✅ En ligne — GPS actif'); setToggling(false) }, async () => { await supabase.from('coursiers').update({ statut: 'disponible' }).eq('id', user.id) setDisponible(true); startGpsTracking(user.id); toast('En ligne sans GPS précis', { icon: '⚠️' }); setToggling(false) } ) } else { stopGpsTracking() await supabase.from('coursiers').update({ statut: 'hors_ligne' }).eq('id', user.id) setDisponible(false); toast('Hors ligne — GPS arrêté', { icon: '🔴' }); setToggling(false) } } catch { setToggling(false) } }
const accepterCourse = async (l: Livraison) => { if (!user) return try { const res = await fetch('/api/coursier/livraisons/accepter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ livraison_id: l.id, coursier_id: user.id }), }) if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.message || 'Erreur serveur') } toast.success('🎉 Course acceptée !') await Promise.all([loadCoursesDisponibles(), loadCoursesEnCours(user.id)]) setTab('en_cours') } catch (e: unknown) { toast.error(e instanceof Error ? e.message : 'Erreur') } }
const handleProposerPrix = async (livraisonId: string) => { if (!user) return const montantStr = propositionInputs[livraisonId] const montant = parseFloat(montantStr) if (!montant || montant < 500) { toast.error('Montant minimum 500 XOF'); return } setPropositionLoading(livraisonId) try { const res = await fetch('/api/coursier/livraisons/accepter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ livraison_id: livraisonId, coursier_id: user.id, action: 'proposer_prix', montant_propose: montant }), }) if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.message || 'Erreur') } toast.success('✅ Proposition envoyée !') setProposantId(null) setPropositionInputs(prev => { const n = { ...prev }; delete n[livraisonId]; return n }) } catch (e: unknown) { toast.error(e instanceof Error ? e.message : 'Erreur envoi proposition') } finally { setPropositionLoading(null) } }
// ── Mise à jour statut — avec support preuve de livraison ──────────────── const updateStatut = async (livraisonId: string, newStatut: string) => { if (!user) return try { const preuveFile = preuveFiles[livraisonId]
 let res: Response

  if (newStatut === 'livree' && preuveFile) {
    // Avec photo de preuve : multipart/form-data
    const formData = new FormData()
    formData.append('livraison_id', livraisonId)
    formData.append('statut', newStatut)
    formData.append('coursier_id', user.id)
    formData.append('preuve_photo', preuveFile)

    res = await fetch('/api/coursier/livraisons/statut', {
      method: 'POST',
      body: formData,
    })
  } else {
  ...
  if (newStatut === 'livree') {
    ...
    if (data.preuve_uploadee) toast.success('📸 Preuve de livraison enregistrée !')
    await loadWallet(user.id)
    // Nettoyer la preuve
    setPreuveFiles(prev => { const n = { ...prev }; delete n[livraisonId]; return n })
  } else {
...
const isVerifie = coursier?.statut_verification === 'verifie' const unreadCount = notifications.filter(n => !n.lu).length const gainsDuJour = transactions .filter(tx => new Date(tx.created_at).toDateString() === new Date().toDateString() && tx.type === 'gain' && tx.status === 'completed') .reduce((s, tx) => s + tx.montant, 0) const missionsVisible = showAllMissions ? coursesDisponibles : coursesDisponibles.slice(0, 4)
if (loading) return ( <div className="min-h-screen flex items-center justify-center" style={{ background: '#0f172a' }}> 
🛵
Chargement...
)
return ( <> {showParams && user && ( <ParametresCoursier user={user} coursier={coursier} wallet={wallet} onClose={() => setShowParams(false)} onUpdate={u => setUser(u)} /> )}
 <div className="min-h-screen flex flex-col" style={{ background: '#f8fafc' }}>
    ...
    <main className="flex-1 max-w-2xl mx-auto w-full px-0 sm:px-4 pb-24">
                      ...
                      <div className="px-4 pt-4 pb-2">
                        ...
                        {isProposing && (
                          <div className="mb-2 flex gap-2">
                            ...
                            <button onClick={() => handleProposerPrix(l.id)} disabled={propositionLoading === l.id}
                            ...
                            <button onClick={() => setProposantId(null)}
                        ...
                        {disponible && isVerifie && (
                          <button
                            ...
                            className={`px-4 py-3 rounded-xl font-bold text-sm transition-all active:scale-95 ${
                              isProposing ? 'bg-orange-100 text-orange-700 border-2 border-orange-300' : 'bg-gray-100 text-gray-600 hover:bg-orange-50 hover:text-orange-600'}`}>
                            💬
                          </button>
                        )}
      ...
      {/* ══ EN COURS ══ — MODIFIÉ : ajout CallButton + preuve de livraison ══ */}
      {tab === 'en_cours' && (
        <div className="px-4 sm:px-0 pt-4 space-y-4">
          ...
          {coursesEnCours.length === 0 ? (
            ...
            const clientInfo = (l as Livraison & { client?: { id: string; nom: string; telephone: string; avatar_url: string } }).client
            ...
            return (
              <div key={l.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{ boxShadow: '0 1px 12px rgba(0,0,0,0.06)' }}>
                ...
                <div className="p-4">
                  ...
                  {/* ── CORRECTION AUDIT : Boutons d'action client (appel + téléphone) ── */}
                  <div className="flex items-center justify-between mb-3 text-xs">
                    <span className="text-gray-500">→ <strong className="text-gray-800">{l.destinataire_nom}</strong></span>
                    <div className="flex items-center gap-2">
                      {/* Appel téléphonique natif vers le destinataire */}
                      <a href={`tel:${l.destinataire_tel}`}
                        className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-green-50 text-green-700 font-semibold text-xs hover:bg-green-100 transition-colors">
                        <Phone size={11} />Appeler destinataire
                      </a>
                      {/* ── NOUVEAU : Bouton appel WebRTC vers le client ── */}
                      {clientInfo?.id && user && (
                        <CallButton
                          appelantId={user.id}
                          appelantRole="coursier"
                          destinataireId={clientInfo.id}
                          livraisonId={l.id}
                          variant="mini"
                          title="Appeler le client (VoIP)"
                        />
                      )}
                    </div>
                  </div>

                  {/* ── NOUVEAU : Preuve de livraison (optionnelle, affichée avant confirmation) ── */}
                  {cfg?.next === 'livree' || l.statut === 'en_route_arrivee' ? (
                    <div className="mb-3 p-3 bg-blue-50 rounded-xl border border-blue-100">
                      <p className="text-xs font-semibold text-blue-700 mb-2 flex items-center gap-1.5">
                        📸 Preuve de livraison
                        <span className="text-blue-400 font-normal">(optionnelle mais recommandée)</span>
                      </p>
                      <input
                        ref={el => { preuveFileRefs.current[l.id] = el }}
                        type="file"
                        accept="image/*"
                        className="hidden"
                        onChange={e => {
                          const file = e.target.files?.[0] || null
                          setPreuveFiles(prev => ({ ...prev, [l.id]: file }))
                          if (file) toast.success('📸 Photo sélectionnée — sera envoyée avec la confirmation')
                        }}
                      />
                      <button
                        onClick={() => preuveFileRefs.current[l.id]?.click()}
                        className="flex items-center gap-2 px-3 py-2 bg-white border border-blue-200 rounded-lg text-xs font-semibold text-blue-700 hover:bg-blue-50 transition-colors"
                      >
                        {preuveFiles[l.id]
                          ? <><CheckCircle size={12} className="text-green-500" />{preuveFiles[l.id]!.name.slice(0, 20)}...</>
                          : <><Camera size={12} />Ajouter une photo</>
                        }
                      </button>
                    </div>
                  ) : null}
      ...
      {tab === 'profil' && (
        <div className="px-4 sm:px-0 pt-4 space-y-4">
          ...
          <button
            onClick={async () => { stopGpsTracking(); await supabase.auth.signOut(); router.push('/coursier/login') }}
